...
/* M Saunby, Feb 2023. THIS SEEMS BROKEN.  Just get one page. */
  ...
  for (uint8_t page = 0; page < 1; page++) {
    ...
    Serial.println(tr, 8);
#endif
    // This is required by maybe calc can be done in another core on Spresense.
...
int Adafruit_MLX90640::getFrameData(uint16_t *mlx90640Frame) {
  int status;
...
int Adafruit_MLX90640::CalculateTo(uint16_t *mlx90640Frame, float *framebuf) {
  float emissivity = 0.95;
  float tr = 23.15;
...
void Adafruit_MLX90640::setParams(paramsMLX90640 params){
  _params = params;

 }
  
paramsMLX90640 Adafruit_MLX90640::getParams(){
  return _params;
}
