...
public:
  ...
  int getFrameData(uint16_t *mlx90640Frame);
  void setParams(paramsMLX90640 params);
  paramsMLX90640 getParams();
  int CalculateTo(uint16_t *mlx90640Frame, float *framebuf);

  uint16_t serialNumber[3]; ///< Unique serial number read from device

//These were private in original code
