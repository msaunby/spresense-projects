...
#define COMRATE                (115200)
#define SHOWRATE                (115200)
...
void setup()
{
  ...
  COMSerial.begin(COMRATE);
  ShowSerial.begin(SHOWRATE);
  GBT.init(COMSerial);
  while (!ShowSerial)
        ;
  while (!COMSerial)
  ...
  while (!theSD.begin()) 
    {
      ...
      ShowSerial.println("Insert SD card.");
    }

  ShowSerial.println("Prepare camera");
  /* begin() Defaults, same as without parameters
  err = theCamera.begin(1, CAM_VIDEO_FPS_30, CAM_IMGSIZE_QVGA_H, CAM_IMGSIZE_QVGA_V, CAM_IMAGE_PIX_FMT_YUV422, 7);
  */
  err = theCamera.begin(1, CAM_VIDEO_FPS_120, CAM_IMGSIZE_QVGA_H, CAM_IMGSIZE_QVGA_V, CAM_IMAGE_PIX_FMT_YUV422, 7);
  ...
  ShowSerial.println("Start streaming");
  ...
  ShowSerial.println("Set Auto white balance parameter");
  ...
  ShowSerial.println("Set still picture format");
...
{
  delay(100);
  uint16_t speed = GBT.getSpeed();
  uint16_t state = GBT.getTargetState();
  ...
  if (state)
  {
      ...
      */
      ...
      ShowSerial.println("call takePicture()");
      ...
      if (img.isAvailable())
        {
          ...
          char filename[16] = {0};
          sprintf(filename, "P%06d.JPG", take_picture_count);
          take_picture_count++;
          ShowSerial.print("Save taken picture as ");
          ShowSerial.print(filename);
          ShowSerial.println("");
      ...
      else
        {
          ...
          ShowSerial.println("Failed to take picture");
        }
    }
}
